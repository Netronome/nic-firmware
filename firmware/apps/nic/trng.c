 ...
 */
  ...
  */
__intrinsic void
trng_init_add_delay(const int delay_count)
{
     uint32_t r_ctr;
...
trng_init()
{
    ...
    uint32_t r_trng_base;
    uint32_t r_read_loop;
    uint32_t valid_data;
 ...
 #define NUM_RANDOMS_TO_GET	        64

	local_csr_write(local_csr_active_lm_addr_0, 0);

    trng_init_add_delay(200);

     //; READ DATA
     r_trng_base = (0x6<<16);
     //;alu_shf[r_trng_base, r_trng_base, OR, cl_num, <<26]

     r_read_loop = NUM_RANDOMS_TO_GET;

    do {
        valid_data = 0;
        do {

            trng_init_add_delay(15);
            __asm cls[read, xfr_rd[0], r_trng_base, TH_TRNG_data, 1], ctx_swap[s1]
            if(xfr_rd[0] != 0)
                valid_data = 1;

        } while(valid_data != 1);

        __asm alu[*l$index0++, --, B, xfr_rd[0]]

        r_read_loop = r_read_loop - 1;
    } while(r_read_loop > 0);

     trng_init_add_delay(40);

     // XPB read Alert Status
     __asm ct[xpb_read, xfr_rd[0], r_xpb_address, TRNG_ALERT, 1], ctx_swap[s1]

#ifndef SKIP_TRNG_INIT_WAIT
