...
load_vlan_members(uint32_t pcie, uint16_t vlan_id, __xread uint64_t *members)
{
    ...
    if (vlan_id <= NIC_MAX_VLAN_ID)
        mem_read64(members, (__mem40 void *)
                   &nic_vlan_to_vnics_map_tbl[pcie][vlan_id],
                   sizeof(uint64_t));
    else
        ret = -1;
...
add_vlan_member(uint32_t pcie, uint16_t vlan_id, uint16_t vid)
{
    ...
    mem_read64(&members_r, (__mem40 void *)
               &nic_vlan_to_vnics_map_tbl[pcie][vlan_id], sizeof(uint64_t));
    min_rxb = (members_r >> 58);
    mem_read32(&rxb_r, (__mem40 void*) (bar_base + NFP_NET_CFG_FLBUFSZ), sizeof(rxb_r));
    ...
    mem_write64(&members_w, (__mem40 void *)
                &nic_vlan_to_vnics_map_tbl[pcie][vlan_id], sizeof(uint64_t));
...
remove_vlan_member(uint32_t pcie, uint16_t vid)
{
    ...
    for (vlan = 0; vlan <= NIC_MAX_VLAN_ID; ++vlan) {
        mem_read64(&members_r, (__mem40 void *)
                   &nic_vlan_to_vnics_map_tbl[pcie][vlan], sizeof(uint64_t));
        ...
        if (members) {
            ...
            for (vid_idx = 0; NFD_MAX_VFS && vid_idx < NFD_MAX_VFS; vid_idx++) {
                if (members & (1ull << vid_idx)) {
                    mem_read32(&rxb_r,
                               (__mem40 void*) (nfd_cfg_bar_base(pcie, vid_idx) +
