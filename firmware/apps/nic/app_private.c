...
{
    return LS_READ(vs_current, vid);
}

void
set_vs_current(const int pcie, uint32_t vid, uint32_t ls)
{
    LS_CLEAR(vs_current, vid);
    vs_current[LS_IDX(vid)] |= (ls << LS_SHF(vid));
}
...
{
    return LS_READ(ls_current, vid);
}

void
set_ls_current(const int pcie, uint32_t vid, uint32_t ls)
{
    LS_CLEAR(ls_current, vid);
    ls_current[LS_IDX(vid)] |= (ls << LS_SHF(vid));
}

/*Interface to vf_lsc_list*/
uint32_t
get_vf_lsc(const int pcie, uint32_t port, uint32_t vid)
{
    return LS_READ(vf_lsc_list[port], vid);
}

void
set_vf_lsc(const int pcie, uint32_t port, uint32_t vid, uint32_t ls)
{
    LS_CLEAR(vf_lsc_list[port], vid);
    vf_lsc_list[port][LS_IDX(vid)] |= (ls << LS_SHF(vid));
}
