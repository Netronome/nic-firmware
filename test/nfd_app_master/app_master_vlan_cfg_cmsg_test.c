...
void main() {
    ...
    for (pass = 0; pass < 8; pass++) {
        ...
        actions[0].instr = INSTR_RX_VEB;
        actions[0].param = mtu + NET_ETH_LEN + 1;
        actions[1].instr = INSTR_STRIP_VLAN;
        actions[2].instr = INSTR_TX_HOST;
        actions[2].param = NFD_VID2QID(pass, 0);
        actions[2].pipeline = SET_PIPELINE_BIT(INSTR_STRIP_VLAN, INSTR_TX_HOST);
        actions[3].value = 0;
        ...
        actions_exp[2].param = NFD_VID2QID(pass, 0);
        actions_exp[2].pipeline = SET_PIPELINE_BIT(INSTR_STRIP_VLAN, INSTR_TX_HOST);
