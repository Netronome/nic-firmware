...
class Unit_dict(object):
    ...
    def __init__(self, group, dut_t_x, a_t, ping_dut, ping_a_t, prefix=''):
        ...
        def _csum_rx_tnl_hdr_tn(tunnel, ipv4, ipv4_opt, ipv6_rt, ipv6_hbh,
                                l4_type, ip_err, l4_err, tail):
           """generate the csum_rx test name"""
           l4_str = '_%s' % l4_type
           l4err_str = '_%serr' % l4_type
           return "csum_rx%s%s%s%s%s%s%s%s%s" % ("_%s" % tunnel,
                                                 "_ipv4" if ipv4 else "_ipv6",
                                                 "_opt" if ipv4_opt else "",
                                                 "_rt" if ipv6_rt else "",
                                                 "_hbh" if ipv6_hbh else "",
                    ...
                    for ip_err in [False, True]:
                        if l4_type == 'udp' or l4_type == 'tcp':
                            for l4_err in [False, True]:
                                # An IPv4 test
                                tn = self.prefix + \
                                     _csum_rx_tnl_hdr_tn(tunnel, ipv4,
                                                         ipv4_opt, ipv6_rt,
                                                         ipv6_hbh, l4_type,
                                                         ip_err, l4_err,
                                                         tail)
                            ...
                            l4_err = False
                            tn = self.prefix + \
                                 _csum_rx_tnl_hdr_tn(tunnel, ipv4, ipv4_opt,
                                                     ipv6_rt, ipv6_hbh,
                                                     l4_type, ip_err,
                                                     l4_err, tail)
                            summary = _csum_rx_tnl_hdr_sum(UnitIPv4,
...
# Unit Tests
###########################################################################
class NFPFlowNICUnit_n2_port(_NFPFlowNIC_nport):
    """Unit tests for the NFPFlowNIC Software Group"""
    ...
    def __init__(self, name, cfg=None,
        ...
        _NFPFlowNIC_nport.__init__(self, name, cfg=cfg, quick=quick,
...
class NFPFlowNICUnit_n4_port(_NFPFlowNIC_nport):
    """Unit tests for the NFPFlowNIC Software Group"""
...
class NFPFlowNICUnit_n5_port(_NFPFlowNIC_nport):
    """Unit tests for the NFPFlowNIC Software Group"""
...
class NFPFlowNICUnit_n8_port(_NFPFlowNIC_nport):
    """Unit tests for the NFPFlowNIC Software Group"""
