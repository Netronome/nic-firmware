...
move(pkt_vec[3], PROTO_MPLS_IPV4_TCP)
...
move(expected[3], PROTO_MPLS_IPV4_TCP)
move(expected[4], 0x0)
...
move(expected[5],  0x00360001) // Total Length = PV Packet Length - 14, ID += 1
...
move(expected[11], 0x0001ffff) // Seq num = 1 (TCP_SEQ += (mss * (lso_seq - 1)))
move(expected[12], 0xffff51f2) // LSO_END = 0, so clear FIN, RST, PSH
...
alu[value, --, B, 17, <<8]
alu[--, value, OR, 1, <<7]
mem[read32, $pkt_rd[0], addr, <<8, tmp, max_18], ctx_swap[s], indirect_ref
