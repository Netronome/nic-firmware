...
error#:
    ...
    .if (loop_cnt == 0)
        move(expected[3], PROTO_IPV6_UDP_VXLAN_IPV4_UDP)
        move(expected[5], ((_PV_OUTER_L3_OFFSET << 24) | (_PV_OUTER_L4_OFFSET << 16) | \
                       (_PV_INNER_L3_OFFSET <<  8) | (_PV_INNER_L4_OFFSET <<  0)))

    .elif (loop_cnt == 1)
        move(expected[3], PROTO_IPV6_UDP_VXLAN_IPV4_UNKNOWN)
