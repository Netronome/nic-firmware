...
#define_eval _PV_L4_OFFSET (14 + 4 + 40)
...
move(pkt_vec[3], PROTO_MPLS_IPV6_TCP)
...
move(expected[3], PROTO_MPLS_IPV6_TCP)
move(expected[4], 0x0)
...
move(expected[5],  0xffff001a) // Total Length = PV Packet Length(0x54) - (14 + 4 + 40)
...
move(expected[16], 0x0001ffff) // Seq num = 1 (TCP_SEQ += (mss * (lso_seq - 1)))
move(expected[17], 0xffff51f2) // LSO_END = 0, so clear FIN, RST, PSH
...
alu[value, --, B, 20, <<8]
alu[--, value, OR, 1, <<7]
mem[read32, $pkt_rd[0], addr, <<8, tmp, max_21], ctx_swap[s], indirect_ref
